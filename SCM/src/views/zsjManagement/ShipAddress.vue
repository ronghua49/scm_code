...
export default {
  ...
  methods: {
    watchList(){
      ...
      // this.pageResult.rows = this.inputList[0].address === '' || this.inputList.length<=0 ?[]:this.inputList;
    },
    handleAdd: function() {
            ...
            if(!this.operation){
              res.rows = res.rows.reduce((final, item, index, arr)=>{
                if(index == params.index){
                  ...
                  arr[index].city = params.city;
                  arr[index].district = params.district;
                  arr[index].headPhone = params.headPhone;
                  arr[index].phone = params.phone;
                  arr[index].postcode = params.postcode;
                  arr[index].province = params.province;
                  arr[index].street = params.street;
                  arr[index].contactPersion = params.contactPersion;
                }
                final = arr;
                return final;
              },[])
