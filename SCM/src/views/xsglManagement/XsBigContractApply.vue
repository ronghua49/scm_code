    ...
    <div class="bigcon-apply">
        ...
        <el-form v-show="!isFenPei"
            ...
            <el-form-item label="合同编号" prop="contractCode">
                <el-input disabled v-model="postData.contractCode" placeholder="请输入内容"></el-input>
            </el-form-item>
            ...
            <el-form-item label="计划发货时间" prop="sendProductDate">
                <div class="block">
                    <el-date-picker style="width: 100%"
                                    v-model="postData.sendProductDate"
            ...
            <el-form-item label="签订时间" prop="dateOfSigning">
                <div class="block">
                    <el-date-picker style="width: 100%"
                                    v-model="postData.dateOfSigning"
                                    value-format="yyyy-MM-dd HH:mm:ss"
                                    type="date"
                                    placeholder="选择日期">
            ...
            <el-form-item label="特求意见" style="width: 96%;">
                <el-input type="textarea" :rows="2"
                          placeholder="请输入内容"
                          v-model="postData.specialOpinion">
        ...
        <el-table v-loading="loading1" :data="tableData1.rows" border stripe element-loading-text="加载中"
                  style="width: 99%" max-height="470">
            ...
            <el-table-column prop="参考信息" key="参考信息" label="参考信息"
            ...
            <el-table-column prop="isSMS" key="SMS" label="SMS" header-align="center" align="center" min-width="100">
                <template slot-scope="scope">
                    <el-checkbox true-label="1" false-label="0"
                                 :disabled="componentParams.flag == 'details'||componentParams.flag == 'approval'"
            ...
            <el-table-column prop="申请" key="申请" label="申请"
                             align="center" min-width="100">
                <el-table-column prop="theNumber" key="数量" label="数量" header-align="center" align="center"
                                 min-width="100">
                    <template slot-scope="scope">
                        <el-input @input.native="numberChanng(scope.row,scope.$index)"
                                  size="small" v-model="scope.row.theNumber"
                                  :disabled="componentParams.flag == 'details'||componentParams.flag == 'approval'"
                                  placeholder="请输入数量">
                ...
                <el-table-column prop="sumPrice" key="贷款金额" label="贷款金额" header-align="center" align="center"
                                 min-width="100"></el-table-column>
            </el-table-column>
        </el-table>
      ...
      ></el-pagination>
        ...
        <el-table v-loading="loading2"
                  :data="tableData3"
                  border element-loading-text="加载中"
                  style="width: 99%"
                  max-height="250">
        ...
        methods: {
            ...
            numberChanng(row, index) {
                ...
                if (row.theNumber) {
                    ...
                    _this.tableData1.rows[index].packageNum = row.theNumber;
                } else {
            ...
            getId() {
                ...
                if (self.componentParams.flag === "apply") {
                    ...
                    self.componentParams.title = "商业大合同申请";
                    this.getBigCode();
                }
                ...
                if (self.componentParams.flag === "details") {
                    ...
                    self.componentParams.title = "商业大合同详情";
                    self.listDetails(self.componentParams.id);
                }
                ...
                if (self.componentParams.flag === "approval") {
                    self.componentParams.title = "商业大合同审批";
                    self.listDetails2(self.componentParams.id);
                }
            ...
            listDetails(id) {
                ...
                _this.$api.base
                    .getScm({
                        url: "/scm/scmXsBigContract/v1/get/" + id
                    })
                    .then(res => {
                        // console.log("详情", res);
                        this.postData = res;
                    ...
                    this.$nextTick().then(() => {
                        if (_this.componentParams.flag != "details") {
                            this.getAgreementDetails(id);
                        } else {
                            this.tableData1.rows = [];
                            this.tableData1.rows = this.postData.scmXsBigContractProductList;
                        }
                    })
                });
            },
            ...
            listDetails2(id) {
                ...
                _this.$api.base
                    .getScm({
                        url: "/scm/scmXsBigContract/v1/getByApplyId/" + id
                    })
                    .then(res => {
                        if (res.state == false) {
                            this.$message({
                        ...
                        for (var i in this.postData) {
                            this.postData[i] = res.value[i]
                            if (this.planA.hasOwnProperty(i)) {
                                this.planA[i] = res.value[i];
                            }
                        }
                        // this.
                        for (var j in this.planB) {
                            this.planB[j] = res.value[j]
                        }
                        this.tableData1.rows = [];
                        this.tableData1.rows = res.value.scmXsBigContractProductList;
                    });
            },
            ...
            applyCache() {
                let _this = this,
                    data = {},
                    url;
                let arr = JSON.stringify(_this.tableData1.rows);
                _this.postData.scmXsBigContractProductList = JSON.parse(arr);
                _this.postData.scmXsBigContractProductList.forEach((v, i) => {
                    if (v.suppyType == '正常供应') {
                        v.suppyType = '1'
                    } else {
                        v.suppyType = '0'
                ...
                data = _this.postData;
                //大合同
                url = "/scm/scmXsBigContract/v1/save";
            ...
            submitForm(formName) {
                ...
                _this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        _this.postData.scmXsBigContractProductList = _this.tableData1.rows;
                        _this.postData.scmXsBigContractProductList.forEach((v, i) => {
                            if (v.suppyType == '正常供应') {
                                v.suppyType = '1'
                            } else {
                        ...
                        _this.sendApplyFunc({
                            data: _this.postData,
                            url: "/scm/scmXsBigContract/v1/sendApply"
                        });
