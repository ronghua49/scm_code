    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="110px" :rules="dataFormRules" ref="dataForm" :size="size">
        ...
        <el-form-item label="收票方" prop="getTicket">
          <el-input v-model="dataForm.getTicket" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      size: "small",
      columns: [
        { prop: "getTicket", label: "收票方", minWidth: 120 },
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        getTicket: params.row.getTicket,
    ...
    handleDelete (data){
      if(data.params.length == 1 && data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdGetticket/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({ 
            url: '/scm/scmZdGetticket/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
            ...
            this.$api.dict.addDict({
              url: '/scm/scmZdGetticket/v1/save',
              data: params
            }).then(res => {
