<template>
    ...
    <div>
      ...
      <scm-table   :highlightCurrentRow="true" :stripe="true" :border="true" :useClickRow="true"
        :data="pageResult" :columns="columns" :showBatchDelete="false" 
        @findPage="findPage" @handleClickRow="handleClickRow" @handleEdit="handleEdit"  @handleDelete="handleDelete">
      </scm-table>
      <div class="forSplit">省区字典列表</div>
      <pro-dic-mannage :devision="devision"></pro-dic-mannage>
      <!--新增编辑界面-->
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size">
        ...
        <el-form-item label="商务分区" prop="businessDivision">
          <el-input v-model="dataForm.businessDivision" auto-complete="off"></el-input>
        </el-form-item>
...
import ProvinceDictManagement from "./ProvinDicManagement"
export default {
  name: "busidicmanagement",
  components: {
    "scm-table": ScmTable,
    "pro-dic-mannage": ProvinceDictManagement
  },
  data() {
    return {
      size: "small",
      columns: [
        { prop: "businessDivision", label: "商务分区", minWidth: 120 },
      ...
      dataFormRules: {
        businessDivision: [
          { required: true, message: "请输入商务分区", trigger: "blur" },
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        businessDivision: params.row.businessDivision,
    ...
    handleDelete (data){
      if(data.params.length == 1 &&  data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdBusinessDivision/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({
            url: '/scm/scmZdBusinessDivision/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
            ...
            this.$api.dict.addDict({
              url: '/scm/scmZdBusinessDivision/v1/save',
              data: params
            }).then(res => {
