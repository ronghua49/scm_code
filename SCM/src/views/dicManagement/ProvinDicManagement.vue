...
export default {
  ...
  data() {
    return {
      size: "small",
      columns: [
        { prop: "province", label: "省区", minWidth: 120 },
      ...
      dataFormRules: {
        province: [
          { required: true, message: "请输入省区", trigger: "blur" },
  ...
  methods: {
    handleAdd: function() {
      ...
      this.dataForm.businessDivisionId = this.devision;
    },
    // 显示编辑界面
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        id: params.row.id,
        businessDivisionId: params.row.businessDivisionId
      });
    },
    //删除
    handleDelete (data){
      if(data.params.length == 1 &&  data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdProvince/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({
            url: '/scm/scmZdProvince/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
