<template>
    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="60px" :rules="dataFormRules" ref="dataForm" :size="size">
        ...
        <el-form-item label="名称" prop="dutyAffirm">
          <el-input v-model="dataForm.dutyAffirm" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      size: "small",
      columns: [
        { prop: "dutyAffirm", label: "名称", minWidth: 120 },
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        dutyAffirm: params.row.dutyAffirm,
    ...
    handleDelete (data){
      if(data.params.length == 1 && data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdDutyAffirm/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({ 
            url: '/scm/scmZdDutyAffirm/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
            ...
            this.$api.dict.addDict({
              url: '/scm/scmZdDutyAffirm/v1/save',
              data: params
            }).then(res => {
    ...
    findPage(data) {
      ...
      this.$api.dict.getDict({
          url:'/scm/scmZdDutyAffirm/v1/list',
          data:{
