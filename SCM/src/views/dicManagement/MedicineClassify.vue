<template>
    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size">
        ...
        <el-form-item label="药品分类" prop="medicineClassify">
          <el-input v-model="dataForm.medicineClassify" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      size: "small",
      columns: [
        { prop: "medicineClassify", label: "药品分类", minWidth: 120 },
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        medicineClassify: params.row.medicineClassify,
    ...
    handleDelete (data){
      if(data.params.length == 1 && data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdMedicineClassify/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({ 
            url: '/scm/scmZdMedicineClassify/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
            ...
            this.$api.dict.addDict({
              url: '/scm/scmZdMedicineClassify/v1/save',
              data: params
            }).then(res => {
