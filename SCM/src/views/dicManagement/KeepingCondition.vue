<template>
    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="110px" :rules="dataFormRules" ref="dataForm" :size="size">
        ...
        <el-form-item label="贮存条件" prop="keepingCondition">
          <el-input v-model="dataForm.keepingCondition" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="建议贮存库区" prop="suggestWarehouse">
          <el-input v-model="dataForm.suggestWarehouse" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      size: "small",
      columns: [
        { prop: "keepingCondition", label: "贮存条件", minWidth: 120 },
        { prop: "suggestWarehouse", label: "建议贮存库区", minWidth: 120 },
      ...
      dataFormRules: {
        keepingCondition: [
          { required: true, message: "请输入贮存条件", trigger: "blur" },
          { max: 20, message: '长度不能超过20', trigger: "blur"}
        ],
        suggestWarehouse: [
          { required: true, message: "请输入建议贮存库区", trigger: "blur" },
          { max: 20, message: '长度不能超过20', trigger: "blur"}
        ],
        code: [
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.dataForm = Object.assign({}, {
        ...
        keepingCondition: params.row.keepingCondition,
        suggestWarehouse: params.row.suggestWarehouse,
    ...
    handleDelete (data){
      if(data.params.length == 1 && data.params[0] != ""){
        this.$api.dict.delDict({
          url: '/scm/scmZdKeepingCondition/v1/remove/'+ data.params[0]
        }).then(data.callback)
      }else{
        if(data.params.length>1){
          ...
          this.$api.dict.delDictPl({ 
            url: '/scm/scmZdKeepingCondition/v1/removes?'+ids,
          }).then(data.callback)
        }else{
          return;
        }
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
            ...
            this.$api.dict.addDict({
              url: '/scm/scmZdKeepingCondition/v1/save',
              data: params
            }).then(res => {
